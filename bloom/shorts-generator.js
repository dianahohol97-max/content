...
const ART_STYLE = `Hand-drawn line animation on warm cream paper (#F6EFE3 — a warm ivory, NEVER grey, taupe or greige): a single fine ink line draws the scene STROKE BY STROKE, live, as if sketched right now. The shot STARTS ON EMPTY PAPER and the drawing finishes exactly at the end of the beat — strokes appear in sequence, one after another, neat and unhurried. This is NOT a finished picture and NOT a photograph: at every moment the drawing is visibly incomplete and still growing. ONE clean confident stroke per edge — no construction lines, no hatching, no repeated or doubled outlines, no sketchy scribble; every line lands once and stays. The drawing is PURELY ADDITIVE: each new stroke is added to what is already on the paper and NOTHING is ever erased, moved, re-proportioned, re-oriented or redrawn. The pose, the viewpoint and the placement on the page are fixed by the very first stroke and never change — the figure must never turn around, flip, or morph into a different angle partway through. Colour arrives only AFTER an outline is closed, as ONE flat even wash of a single brand colour (lavender, blush pink, sage green or deep violet) filling the shape edge to edge — smooth and uniform, never blotchy, never streaked, never patchy with gaps. The subject fills most of the frame — drawn large, not a small object lost in empty paper. The paper stays still: no camera movement, no zoom, no pan. STRICTLY no grey, no charcoal, no black fills. No text, no letters, no numbers. NOT photoreal, NOT 3D, NOT cartoon, NOT childish. Vertical 9:16 composition, subject centred, lower third left empty for captions.`;

// Line art has no lighting to modulate, so the beat's mood rides on how the
// line behaves and how much colour is allowed in.
// People were banned outright, so a beat about a woman burning out got drawn as
// a mirror in a hallway — technically on-brand, saying nothing. When the line is
// about a person, draw the person; the ban only ever existed to avoid the
// uncanny faces image models produce, which a turned head solves just as well.
// Offering several viewpoints ("from behind, in profile, or head lowered")
// alongside a prop she has to face made the model draw a bare back and then
// rotate the whole figure to the front mid-shot. One viewpoint, stated once.
const PERSON_RULE = `When the narration is about a PERSON (she, you, someone doing or feeling something), DRAW THE PERSON — one woman from the waist up, facing the viewer straight on, in the same fine continuous line, doing the thing being described. Her viewpoint is FIXED: she faces forward from the first stroke to the last and never turns. Her face stays BLANK — the head, hair and jawline are drawn, but no eyes, nose or mouth are ever added; the face is left as empty paper. Clothing is simple and covering (a jumper, a tee) — never bare shoulders or an exposed back. One person only, never a crowd. Only when the beat is genuinely about a thing, not a person, draw the object instead.`;
...
const SCENE_SPEC = `    - tag: a SHORT category slug for this scene's main subject (lowercase, underscores) from a small reusable vocabulary so images can be cached/reused. Prefer ONE of: brain, desk_messy, desk_tidy, desk_empty, coffee, journal, window_light, bed, clock, plant, books, phone, path, lamp, calendar, sparks, cozy_room, sky. If none fit, make a simple 1-2 word slug. Scenes about the same thing MUST share the same tag.
    - imagePrompt: ALWAYS begin with exactly this style: "${ART_STYLE}" ${ART_MOOD} Then describe ONE subject that ILLUSTRATES EXACTLY what the narration says during this beat, and SAY IN WHAT ORDER THE LINE DRAWS IT, e.g. "first the mug's rim, then its body, then the curl of steam, last a lavender wash inside". ${PERSON_RULE} Keep it to a few strokes — a subject that cannot be drawn in about five seconds is too complex, so choose fewer elements rather than a busy scene. Each scene draws a DIFFERENT subject than the previous one — never repeat the same item in a row. Leave the lower third bare paper so captions stay readable.
...
- voiceover: FULL narration as one flowing text (${isFlag ? "~110-135 words for ~42s" : "~75-90 words for ~30s"}). Natural spoken rhythm. SPEAK STRAIGHT TO THE VIEWER as "you" — never narrate about a third person ("she aced every exam, she burned out"), because a case study is something to watch happen to somebody else, while "you" is recognition. Say "You aced every exam, held every job, never missed a deadline — and burned out completely by 32." Use "she/they" only for someone genuinely other (a doctor, a researcher, a partner). End with: Follow for daily ADHD content.
