...
const PERSON_RULE = `When the narration is about a PERSON (she, you, someone doing or feeling something), DRAW THE PERSON — one woman in the same fine continuous line, full figure or from the waist up, doing the thing being described. Draw her from behind, in profile, or with her head lowered so no facial features are needed; if the face is visible at all, it is two or three suggested strokes, never detailed eyes or a rendered portrait. One person only, never a crowd. Only when the beat is genuinely about a thing, not a person, draw the object instead.`;
